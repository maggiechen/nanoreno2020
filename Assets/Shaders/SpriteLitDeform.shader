...
Shader "Universal Render Pipeline/2D/Sprite-Lit-Deform"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // Motion over texture and time
                float noiseX = 0.005 * (0.3*sin(i.uv.x * 5 + _Time.w) + sin(_Time.w));
    			float noiseY = 0.005 * (0.3*sin(i.uv.y * 10 + _Time.w) + sin(0.6*_Time.w));
                half4 main = i.color * SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv + float2(noiseX, noiseY));
